...
class CreateServ10ServicesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('serv10_services', function (Blueprint $table) {
            $table->id();
            //Services
            $table->foreignId('category_id')->constrained('serv10_services_categories')->onDelete('cascade');
            ...
            $table->boolean('active_topic')->default(0);

            $table->string('title_gallery')->nullable();
            $table->text('description_gallery')->nullable();
            $table->boolean('active_gallery')->default(0);
