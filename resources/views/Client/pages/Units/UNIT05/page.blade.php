...
@section('content')
    <main id="root" class="unit05-page">
        @if ($section)
            <section class="unit05-page__banner"
                ...
                <menu class="unit05-page__menus__subcategories">
                    <div class="unit05-page__menus__subcategories__swiper-wrapper swiper-wrapper">
                        @foreach ($subcategories as $subcategory)
                            <a {{ $subcategory->title }}
                                href="{{ route('unit05.subcategory.page', ['UNIT05UnitsCategory' => $categorySelected->slug, 'UNIT05UnitsSubcategory' => $subcategory->slug]) }}"
            ...
            <section class="unit05-page__services">
                @foreach ($units as $unit)
                    <article class="unit05-page__services__item" style="position: relative">
                        <a title="{{ $unit->title }}"
                            href="{{ route('unit05.show', ['UNIT05UnitsCategory' => $unit->category->slug, 'UNIT05UnitsSubcategory' => $unit->subcategory->slug, 'UNIT05Units' => $unit->slug]) }}"
                            class="link-full">
                        </a>
                        @if ($unit->path_image_box)
                            <img class="unit05-page__services__item__bg"
                                src="{{ asset('storage/' . $unit->path_image_box) }}"
                            ...
                            <div class="unit05-page__services__item__information">
                                @if ($unit->path_image_icon)
                                    <img class="unit05-page__services__item__information__logo"
                                        src="{{ asset('storage/' . $unit->path_image_icon) }}"
                                        alt="Logo marca do serviço {{ $unit->title }}">
                                @endif
                                @if ($unit->subtitle)
                                    <h4 class="unit05-page__services__item__information__subtitle">{{ $unit->subtitle }}
                                    </h4>
                                @endif
                                @if ($unit->title)
                                ...
                                @if ($unit->links->count())
                                    @foreach ($unit->links as $link)
                                        @if ($link->link)
                                            <a href="{{ getUri($link->link) }}" target="{{ $link->target_link }}"
                                                class="unit05-page__services__item__information__cta">
                                                {{ $link->title }}
                                            </a>
                                        @endif
                                    @endforeach
