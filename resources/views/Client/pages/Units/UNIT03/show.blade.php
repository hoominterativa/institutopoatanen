...
@section('content')
    ...
    @if ($bannerShow)
        <section class="unit03-show__banner"
            ...
            <header
                class="unit03-show__banner__content container d-flex flex-column align-items-center justify-content-center">
    ...
    <section class="unit03-show__top">
        <div class="unit03-show__top__left d-flex flex-column align-items-stretch {{ $contact ? '' : ' w-100' }}">

            <div class="unit03-show__top__upper">
                @if ($unit->path_image_icon_show)
                    <img src="{{ asset('storage/' . $unit->path_image_icon_show) }}" alt=""
                        class="unit03-show__top__img">
                @endif
                            ...
                            @foreach ($socials as $social)
                                <li class="unit03-show__top__social__item">
                                    <a href="{{ $social->link ? getUri($social->link) : 'javascript:void(0)' }}"
                                        target="{{ $social->target_link }}" rel="prev">
            ...
            <div class="unit03-show__top__form">
                ...
                <button type="submit" class="unit03-show__top__form__cta">
                    <img src="{{ asset('storage/uploads/tmp/icon-general.svg') }}"
                        class="unit03-show__top__form__cta__icon" alt="Ícone">
    ...
    @if ($contents->count())
        @foreach ($contents as $content)
            <section class="unit03-show__content"
                        ...
                        @foreach ($content->galleryContents as $galleryContent)
                            <div class="unit03-show__content__carousel__item">
                                @if ($galleryContent->path_image)
                                    <img src="{{ asset('storage/' . $galleryContent->path_image) }}" alt="Carousel image">
                                @endif
                            </div>
                        @endforeach
                    </div>
                    <div class="unit03-show__content__right">
                        ...
                        @if ($content->link_button)
                            <a rel="next"
                                ...
                                target="{{ $content->target_link_button }}" class="unit03-show__content__cta">
