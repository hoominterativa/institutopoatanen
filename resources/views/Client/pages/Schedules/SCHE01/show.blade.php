    ...
    <div class="sche01-show">
        @if ( $bannerShow)
            ...
            style="background-image: url({{ asset('storage/' . $bannerShow->path_image_desktop) }}); background-color: {{$bannerShow->background_color}};">
                <header class="sche01-show__banner__content container d-flex flex-column align-items-center justify-content-center">
                ...
                <article class="sche01-show__content d-flex flex-column">
                    <header class="sche01-show__content__header w-100 d-flex flex-row">
                        <div
                            ...
                            <span class="sche01-show__content__day">{{Carbon\Carbon::parse($schedule->event_date)->formatLocalized('%d')}}</span>
                            <span class="sche01-show__content__month">{{Carbon\Carbon::parse($schedule->event_date)->formatLocalized('%B')}}</span>
                            <span class="sche01-show__content__year">{{Carbon\Carbon::parse($schedule->event_date)->formatLocalized('%Y')}}</span>
                        </div>
                            ...
                            <ul class="sche01-show__content__header__topics">
                                <li class="sche01-show__content__header__topics__item">
                                    @if ($schedule->path_image_hours || $schedule->event_time)
                                        <img src="{{ asset('storage/' . $schedule->path_image_hours) }}" alt="" class="sche01-show__content__header__topics__item__icon">
                                            {{ date('H:i', strtotime($schedule->event_time)) }}
                                    @endif
                                </li>
                                <li class="sche01-show__content__header__topics__item">
                                    @if ($schedule->path_image_sub || $schedule->subtitle)
                                        <img src="{{ asset('storage/' . $schedule->path_image_sub) }}" alt="" class="sche01-show__content__header__topics__item__icon">
                                        {{$schedule->subtitle}}
                                    @endif
                                </li>
                            </ul>
                    ...
                    <main class="sche01-show__content__main d-flex flex-column">
                        @if ($schedule->path_image)
                            <img src="{{ asset('storage/' . $schedule->path_image) }}" alt="" class="sche01-show__content__img">
                        @endif
                        ...
                        @if ($schedule->link_button)
                            <a href="{{getUri($schedule->link_button)}}" target="{{$schedule->target_link_button}}" class="sche01-show__content__cta">
                    ...
                    @if ($contact)
                        <div class="sche01-form d-flex flex-column align-items-start justify-content-start">
                            ...
                            <button type="submit" class="sche01-form__cta">
                                <img src="{{ asset('storage/uploads/tmp/icon-general.svg') }}" class="sche01-form__cta__icon" alt="Ícone">
