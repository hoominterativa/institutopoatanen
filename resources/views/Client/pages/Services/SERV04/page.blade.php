                                ...
                                <div class="row row--sepa mx-auto px-0">
                                    @if ($category->path_image)
                                        <div class="serv04-page__box__image col-auto px-0">
                                            <img src="{{ asset('storage/' . $category->path_image) }}" alt="Image Categoria" loading="lazy">
                                        </div>
                                    @endif
                ...
                <section class="serv04-page__subcategory container-fluid px-0">
                    <div class="container container--seps">
                        <div class="serv04-page__subcategory__nav">
                            <div class="carousel-serv04-page__subcategory px-0 mb-0 owl-carousel serv04-page__subcategory____eng">
                                @foreach ($services as $serviceGet)
                                    <div class="serv04-page__subcategory__box position-relative {{ $serviceGet->id == $service->id ? 'active' : '' }}"
                                        ...
                                        <a href="{{ route('serv04.show', ['SERV04ServicesCategory' => $category->slug, 'SERV04Services' => $serviceGet->slug]) }}" class="link-full"></a>
                                            @if ($serviceGet->path_image_icon)
                                                <div class="serv04-page__subcategory__box__image">
                                                    <img src="{{ asset('storage/' . $serviceGet->path_image_icon) }}" alt="Subcategoria" loading="lazy">
                                                </div>
                                            @endif
                                ...
                                <div class="row mx-auto px-0">
                                    @if ($service->path_image)
                                        <div class="serv04-page__subcategory__content__box__image col-auto px-0">
                                            <img src="{{ asset('storage/' . $service->path_image) }}" alt="Categoria" loading="lazy">
                                        </div>
                                    @endif
                                    <div class="serv04-page__subcategory__content__box__description col">
                                        @if ($service->title || $service->subtitle)
                                            <h2 class="serv04-page__subcategory__content__box__title">{{ $service->title }}</h2>
                                            <h2 class="serv04-page__subcategory__content__box__subtitle">{{ $service->subtitle }}</h2>
                                        @endif
                                        @if ($service->text)
                                        ...
                                        <div class="accordion-item serv04-page__subcategory__content__accordion__boxQuestion">
                                            @if ($topic->title)
                                                <h2 class="accordion-header" id="flush-collapseOne">
                                                    <button class="accordion-button collapsed serv04-page__subcategory__content__accordion__boxQuestion__title"
                                                        type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapse-{{ $topic->id }}"
                                                        aria-expanded="false" aria-controls="flush-collapse-{{ $topic->id }}">
                                                        {{ $topic->title }}
                                                    </button>
                                                </h2>
                                            @endif
