@if ($section)
    <section id="PORT02" class="port02"
                ...
                <nav class="port02__categories">
                    <div class="port02__categories__carousel">
                        @foreach ($categories as $category)
                            <a href="{{ route('port02.category.page', ['PORT02PortfoliosCategory' => $category->slug]) }}"
                                class="port02__categories__link">
                                <div class="port02__categories__item">
                                    <img src="{{ asset('storage/' . $category->path_image_icon) }}" width="62"
                                        alt="{{ $category->title }}" class="port02__categories__item__icon">
                    ...
                    <div class="port02__portfolios__carousel">
                        @foreach ($portfolios as $portfolio)
                            <article class="port02__portfolios__item">
                                <a href="#{{ $portfolio->slug }}" data-fancybox="{{ $portfolio->slug }}"
                                    class="link-full"></a>
                                @if ($portfolio->path_image_box)
                                    <img src="{{ asset('storage/' . $portfolio->path_image_box) }}" width="100%"
                                        height="100%" alt="{{ $portfolio->title }}"
                                        class="port02__portfolios__item_imagem position-absolute">
                                @endif
