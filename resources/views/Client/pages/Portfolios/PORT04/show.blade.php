            ...
            <header class="port04-show__banner container-fluid px-0">
                <div class="port04-show__banner__container">
                    @if ($portfolio->active_banner)
                        <div class="port04-show__banner__image" style="background-image: url({{ asset('storage/' . $portfolio->path_image_desktop_banner) }});  background-color: {{ $portfolio->background_color_banner }};">
                    ...
                    <div class="port04-show__content__main-content">
                        @if ($portfolio->path_image_content)
                            <div class="port04-show__content__main-content__image">
                                <img src="{{ asset('storage/' . $portfolio->path_image_content) }}" alt="Imagem" class="port04-show__content__main-content__image__img">
                            </div>
                        @endif
                                ...
                                <div class="accordion-item">
                                    @if ($additionalTopic->title)
                                        <h4 class="accordion-header" id="heading{{$additionalTopic->id}}">
                                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse{{$additionalTopic->id}}" aria-expanded="true" aria-controls="collapse{{$additionalTopic->id}}">
                                                @if ($additionalTopic->path_image_icon)
                                                    <div class="">
                                                        <img src="{{ asset('storage/' . $additionalTopic->path_image_icon) }}" alt="Imagem" class="me-4">
                                                    </div>
                                                @endif
                                    ...
                                    @if ($additionalTopic->text)
                                        <div id="collapse{{$additionalTopic->id}}" class="accordion-collapse collapse" aria-labelledby="heading{{$additionalTopic->id}}" data-bs-parent="#accordionFlushExample">
                                ...
                                <div class="port04-show__content__topics__item">
                                    @if ($topic->path_image_icon)
                                        <div class="port04-show__content__topics__item__image">
                                            <img src="{{ asset('storage/' . $topic->path_image_icon) }}" alt="Imagem" class="port04-show__content__topics__item__image__img">
                                        </div>
                                    @endif
                                    <div class="port04-show__content__topics__item__information">
                                        @if ($topic->title)
                                            <h4 class="port04-show__content__topics__item__information__title">{{ $topic->title }}</h4>
                                        @endif
                                        @if ($topic->text)
                                ...
                                <div class="port04-show__content__gallery__item">
                                    @if ($gallery->path_image)
                                        <div class="port04-show__content__gallery__item__image">
                                            <img src="{{ asset('storage/' . $gallery->path_image) }}" alt="Imagem" class="port04-show__content__gallery__item__image__img">
                                        </div>
                                    @endif
                                </div>
                            @endforeach
                    ...
                    @if ($section->active_relationship)
                        <header class="port04-show__related-items__header container">
                            @if ($section->title_relationship || $section->subtitle_relationship)
                                <h4 class="port04-show__related-items__header__subtitle">{{ $section->subtitle_relationship }}</h4>
                                <h3 class="port04-show__related-items__header__title animation fadeInLeft">{{ $section->title_relationship }}</h3>
                            ...
                            @foreach ($relationships as $relationship)
                                <article class="port04-show__related-items__content__item">
                                    <a class="d-flex flex-row align-items-end w-100"
                                        ...
                                        <img class="port04-show__related-items__content__item__image" src="{{ asset('storage/' . $relationship->path_image) }}" alt="Imagem do portfólio">
                                        <div class="port04-show__related-items__content__item__container">
                                            ...
                                            <img class="port04-show__related-items__content__item__container__icon transition" src="{{ asset('storage/' . $relationship->path_image_icon) }}" alt="Ícone do portfólio">
                                        </div>
                                    </a>
                                </article>
                            @endforeach
