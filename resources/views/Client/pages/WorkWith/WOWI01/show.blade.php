        ...
        <div id="WOWI01" class="wowi01-show">
            @if ($workWith->path_image_banner || $workWith->title_banner)
                <section class="container-fluid px-0">
                    <header class="wowi01-show__header"
                        style="background-image: url({{ asset('storage/' . $workWith->path_image_banner) }})">
                    ...
                    <div class="wowi01-show__content">
                        <div class="">
                            @if ($workWith->path_image_icon)
                                <img src="{{ asset('storage/' . $workWith->path_image_icon) }}" width="36"
                                    alt="{{ $workWith->title }} {{ $workWith->subtitle }}"
                                    class="wowi01-show__content__icon">
                            @endif
                        ...
                        @if ($topicSection->title || $topicSection->subtitle || $topicSection->description)
                            <header class="wowi01-show__container-box__header">
                                <h3 class="wowi01-show__container-box__header__container">
                                    <span
                                        class="wowi01-show__container-box__header__title">{{ $topicSection->title }}</span>
                                    <span
                                        class="wowi01-show__container-box__header__subtitle">{{ $topicSection->subtitle }}</span>
                                </h3>
                                @if ($topicSection->title || $topicSection->subtitle)
                                    <hr class="wowi01-show__container-box__header__line">
                                @endif
                                <p class="wowi01-show__container-box__header__paragraph">{{ $topicSection->description }}
                                </p>
                            </header>
                        @endif
                            ...
                            @foreach ($topics as $topic)
                                <article class="wowi01-show__container-box__item col-12 col-sm-4 col-lg-3 mb-5">
                                    <div class="content transition">
                                        <a href="{{ $topic->link ?: 'javascript:void(0)' }}"
                                            target="{{ $topic->link_target }}">
                                            @if ($topic->path_image_thumbnail)
                                                <img src="{{ asset('storage/' . $topic->path_image_thumbnail) }}"
                                                    width="100%" height="100%"
                                                    class="wowi01-show__container-box__image d-block"
                                                    alt="{{ $topic->title }}">
                                            @endif
                                                ...
                                                <figure class="wowi01-show__container-box__icon">
                                                    @if ($topic->path_image_icon)
                                                        <img src="{{ asset('storage/' . $topic->path_image_icon) }}"
                                                            class="icon" width="60px" alt="{{ $topic->title }}">
                                                    @endif
                                                </figure>
            ...
            <section class="wowi01-show__content-section container-fluid">
                <div class="container wowi01-show__content-section__container">
                    @if (
                        $workWith->path_image_content ||
                            $workWith->title_content ||
                            $workWith->subtitle_content ||
                            $workWith->description_content)
                        <div class="wowi01-show__content-section__info row">
                            @if ($workWith->path_image_content)
                                <div class="wowi01-show__content-section__info__image col-12 col-lg-6">
                                    <img src="{{ asset('storage/' . $workWith->path_image_content) }}" width="429"
                                        class="wowi01-show__content-section__info__image__item" alt="Título Subtitulo">
                                </div>
                            @endif
                            <div
                                class="wowi01-show__content-section__info__description col-12 {{ $workWith->path_image_content ? 'col-lg-6' : '' }}">
                                <h3 class="wowi01-show__content-section__info__container">
                                    <span
                                        class="wowi01-show__content-section__info__title">{{ $workWith->title_content }}</span>
                                    <span
                                        class="wowi01-show__content-section__info__subtitle">{{ $workWith->subtitle_content }}</span>
                                </h3>
                                <hr class="wowi01-show__content-section__info__line">
                                <p class="wowi01-show__content-section__info__paragraph">
                                    {{ $workWith->description_content }}</p>
                                @if ($workWith->link_content)
                                    <a href="{{ getUri($workWith->link_content) }}"
                                        target="{{ $workWith->link_target_content }}"
                                        class="wowi01-show__content-section__info__cta float-end d-flex align-items-center justify-content-center">
                                        <img src="{{ asset('storage/uploads/tmp/icon-general.svg') }}" width="25px"
                                            class="me-3 wowi01-show__content-section__info__cta__icon">
                                        CTA
                                    </a>
                                @endif
                            </div>
                    ...
                    <div class="wowi01__container-box carousel-wowi01 row">
                        @foreach ($workWiths as $workWith)
                            <article class="wowi01__container-box__item col-12 col-sm-4 col-lg-3 mb-5">
                                <div class="content transition">
                                    <img src="{{ asset('storage/uploads/tmp/image-box.jpg') }}" width="100%"
                                        height="100%" class="position-absolute top-0 start-0"
                                        alt="{{ $workWith->title }}">
                                    <a href="{{ route('wowi01.show', ['WOWI01WorkWith' => $workWith->slug]) }}">
                                        <div
                                            class="wowi01__container-box__info d-flex flex-column justify-content-center align-items-center">
                                            @if ($workWith->path_image_icon)
                                                <figure class="wowi01__container-box__image">
                                                    <img src="{{ asset('storage/' . $workWith->path_image_icon) }}"
                                                        class="icon" width="60px" alt="{{ $workWith->title }}">
                                                </figure>
                                            @endif
                                            <div class="wowi01__container-box__description">
                                                ...
                                                <p class="wowi01__container-box__paragraph mx-auto">
                                                    {{ $workWith->description }}</p>
                                            </div>
                                        </div>
                                    </a>
                                    <a href="{{ route('wowi01.show', ['WOWI01WorkWith' => $workWith->slug]) }}"
                                        class="wowi01__container-box__link d-flex align-items-center justify-content-center">
                                        <img src="{{ asset('storage/uploads/tmp/icon-general.svg') }}" width="25px"
                                            class="me-3 wowi01__container-box__link__icon">
                                        CTA
                                    </a>
                                </div>
                            </article>
