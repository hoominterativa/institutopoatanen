@if ($slides->count())
    <section id="SLID01" class="slid01">
        <div class="slid01__swiper-wrapper swiper-wrapper">
            {{-- BACKEND: RETIRAR CAMPO DE POSICIONAMENTO DA IMAGEM DO PAINEL ($slide->position_content) --}}
            {{-- BACKEND: Comentei no controller so pra fazer o teste das imagens --}}
            @foreach ($slides as $slide)
                <div class="slid01__item swiper-slide">
                        ...
                        <picture class="slid01__item__background">
                            @if ($slide->path_image_mobile)
                                <source media="(max-width:991.98px )"
                                    srcset="{{ asset('storage/' . $slide->path_image_mobile) }}">
                            @endif

                            @if ($slide->path_image_desktop)
                                <img src="{{ asset('storage/' . $slide->path_image_desktop) }}"
                                    class="slid01__item__background__img"
                        ...
                        <header class="slid01__item__header">
                            ...
                            @if ($slide->link_button)
                                <a href="{{ getUri($slide->link_button) }}" target="{{ $slide->target_link_button }}"
                                    class="slid01__item__header__cta">
                    ...
                    @if ($slide->path_image_png)
                        <img
                        class="slid01__item__image"
                        src="{{ asset('storage/' . $slide->path_image_png) }}"
