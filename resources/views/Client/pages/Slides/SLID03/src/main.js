...
$(".slid03-show__form__additional__add").on("click", function () {
    ...
    $.ajax({
        headers: {
            "X-CSRF-TOKEN": $(
                ".slid03-show__form__item input[name=_token]"
            ).val(),
        },
        type: "POST",
        url: url,
        success: function (data) {
            ...
            $(".slid03-show__form__additional__item")
                .first()
                .find(
                    "input:not(input[type=hidden], input[type=checkbox], input[type=radio]), textarea"
                )
                .each(function (elem) {
                    var that = $(this),
                    ...
                    if (placeholder) {
                        ...
                        that.parent().append(`
                        <div class="form-placeholder"><label for="${
                            name + additionalLength
                        }" class="placeholder--custom">${placeholder}</label></div>
                    `);
