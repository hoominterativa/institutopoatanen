        ...
        <div class="lightbox-gall03__bottom">
            <div class="lightbox-gall03__bottom__main">
                @if ($gallery->path_image)
                    <img src="{{ asset('storage/' . $gallery->path_image) }}" width="100%" height="100%" class="lightbox-gall03__bottom__main__item" alt="">
            ...
            <div class="lightbox-gall03__bottom__thumbnail">
                <div class="lightbox-gall03__bottom__thumbnail__carousel">
                    @if ($gallery->path_image)
                        <img src="{{ asset('storage/' . $gallery->path_image) }}" alt="Imagem" data-main-image="{{ asset('storage/' . $gallery->path_image) }}" data-main-title=" {{$gallery->image_legend}} ">
                    @endif
                    @foreach ($gallery->images as $image)
                        <img src="{{ asset('storage/' . $image->path_image) }}" alt="Imagem" data-main-image="{{ asset('storage/' . $image->path_image) }}" data-main-title=" {{$gallery->image_legend}} ">
                    @endforeach
                </div>
            </div>
        </div>
