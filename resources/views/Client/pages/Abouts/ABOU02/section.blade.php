<section id="ABOU02" class="abou02 container-fluid">
    <div class="row">
        <div class="abou02__boxLeft">
            <div class="abou02__description">
                <h4 class="abou02__subtitle">subtitulo</h4>
                <h5 class="abou02__title">Titulo</h5>
                <p>
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras vel tortor
                    eu purus gravida sollicitudin vel non libero. Vivamus commodo porta 
                    velit, vel tempus mi pretium sed. In et arcu eget purus mattis posuere. Donec tincidunt dignissim faucibus. 
                </p>
            </div>
        </div>
        <div class="abou02__boxRight">
            <div class="carrossel_abou02">
                <article class="abou02__box__item">
                    <div class="abou02__image">
                        <img src="" class="fluid" alt="Imagem Titulo">
                    </div>
                    <div class="abou02__box__description">
                        <h4 class="abou02__box__subtitle">subtitulo</h4>
                        <h5 class="abou02__box__title">Titulo</h5>
                    </div>
                </article>
            </div>
        </div>
    </div>
</section>
{{-- END .abou02 --}}