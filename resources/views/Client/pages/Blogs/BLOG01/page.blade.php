                        ...
                        <article class="blog01-page__boxs__featured__item">
                            <div itemscope itemtype="http://schema.org/Article" class="blog01-page__boxs__featured__item__content transition row align-items-center">
                                ...
                                <div class="blog01-page__boxs__featured__item__description col-12 col-sm-6">
                                    ...
                                    <a itemprop="url" href="{{route('blog01.show.content', ['BLOG01BlogsCategory' => $blogFeatured->category->slug, 'BLOG01Blogs' => $blogFeatured->slug])}}">
                                        <h2 itemprop="name" class="blog01-page__boxs__featured__item__title">{{$blogFeatured->title}}</h2>
                                    </a>
                                    <span class="blog01-page__boxs__featured__item__date-publish">
                                        Data: <span itemprop="datePublished" content="{{$blogFeatured->publishing}}" class="blog01-page__boxs__featured__item__date">{{Carbon\Carbon::parse($blogFeatured->publishing)->formatLocalized('%d de %B de %Y')}}</span>
                                    </span>
                                    <p itemprop="articleBody" class="blog01-page__boxs__featured__item__paragraph">{{$blogFeatured->description}}</p>
                                    <a itemprop="url" href="{{route('blog01.show.content', ['BLOG01BlogsCategory' => $blogFeatured->category->slug, 'BLOG01Blogs' => $blogFeatured->slug])}}" class="blog01-page__boxs__featured__item__cta d-flex align-items-center justify-content-center ms-auto">
                            ...
                            <a itemprop="url" href="{{route('blog01.show.content', ['BLOG01BlogsCategory' => $blog->category->slug, 'BLOG01Blogs' => $blog->slug])}}">
                                ...
                                <div class="blog01-page__boxs__item__description">
                                    <span class="blog01-page__boxs__item__date-publish">
                                        Data: <span itemprop="datePublished" content="{{$blog->publishing}}" class="blog01-page__boxs__item__date">{{Carbon\Carbon::parse($blog->publishing)->formatLocalized('%d de %B de %Y')}}</span>
                                    </span>
                                    <h3 itemprop="name" class="blog01-page__boxs__item__title">{{$blog->title}}</h3>
                                    <div class="d-flex align-items-center justify-content-between">
                                        <p itemprop="articleBody" class="blog01-page__boxs__item__paragraph">{{$blog->description}}</p>
                                        <img src="{{asset('storage/uploads/tmp/icon-general.svg')}}" width="34" class="blog01-page__boxs__item__icon ms-3" alt="{{$blog->title}}"/>
                                    </div>
                                </div>
                            </a>
                        </div>
