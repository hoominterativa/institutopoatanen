        ...
        <section id="CONT10V1" class="cont10v1 container-fluid px-0"
            ...
            <div class="container container--pd">
                <header class="cont10v1__encompass text-center">
                    @if ($content->title)
                        <h4 class="cont10v1__encompass__title">{{ $content->title }}</h4>
                    @endif
                    @if ($content->subtitle)
                        <h5 class="cont10v1__encompass__subtitle">{{ $content->subtitle }}</h5>
                    @endif
                </header>
                            ...
                            <!-- <div class="cont10v1__content__list__item flex-column"> -->
                                @foreach ($content->topics as $topic)
                                    <div class="cont10v1__content__list__item__box d-flex">
                                        @if ($topic->date)
                                            <div class="cont10v1__content__list__item__title">{{ Carbon\Carbon::parse($topic->date)->format('d/m/Y') }}</div>
                                        @endif
                                        @if ($topic->locale)
                                            <div class="cont10v1__content__list__item__title">{{ $topic->locale }}</div>
                                        @endif
                                        @if ($topic->link_button)
                                            <div class="cont10v1__content__list__item__title">
                                                <a href="{{ getUri($topic->link_button) }}" target="{{ $topic->link_target_button }}" class="cont10v1__content__cta transition d-flex justify-content-center align-items-center">
