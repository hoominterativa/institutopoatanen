        ...
        @if ($content)
            <main class="cont04__main">
                @if ($content->path_image)
                    <img src="{{ asset('storage/' . $content->path_image) }}" loading="lazy" alt="Imagem de apoio do {{ $content->title }}" class="cont04__main__image animation fadeInLeft">
                @endif
                ...
                <div class="cont04__main__information">
                   {{-- @if ($content->subtitle)
                    ...
                    @if ($content->title)
                        <h4 class="cont04__main__information__title animation fadeInRight">{!! $content->title !!}</h4>
                    @endif
                   ...
                   {{-- @if ($content->description)
                    ...
                    @if ($content->link_button)
                        <a href="{{ getUri($content->link_button) }}" target="{{ $content->target_link_button }}"
                            class="cont04__main__information__cta animation fadeInRight">
