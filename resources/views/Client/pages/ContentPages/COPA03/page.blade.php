    ...
    @if ($contentPage)
        <main id="root">
            <section class="copa03__banner"
                ...
                @if ($categories->count())
                    <div class="copa03__banner__categories quedinha">
                        <button class="copa03__banner__categories__btn quedinha__btn">{{$categoryCurrent->title ?? 'Categorias'}}</button>
                        <ul class="copa03__banner__categories__content quedinha__content">
                            @foreach ($categories as $category)
                                <li class="copa03__banner__categories__content__item">
                                    <a href="{{ route('copa03.category.page', ['COPA03ContentPages' => $contentPage->slug, 'COPA03ContentPagesCategory' => $category->slug]) }}"
                                        class="copa03__banner__categories__content__item__link">
                                        {{ $category->title }} </a>
                                </li>
                            @endforeach
                        </ul>
            ...
            @if ($subcategoryTopics->count())
                <section class="copa03__topics"  id="topicos">
                    ...
                    <nav class="copa03__topics__subcategories">
                        <ul class="copa03__topics__subcategories__swiper-wrapper swiper-wrapper">
                            @foreach ($subcategoryTopics as $subcategoryTopic)
                                <li class="copa03__topics__subcategories__item swiper-slide {{-- {{$subcategoryTopic->id == $subcategoryCurrentT->id ? 'active' : ''}} --}}">
                                    <a href="{{ route('copa03.subcategory.page', [
                                        'COPA03ContentPages' => $contentPage->slug,
                                        'COPA03ContentPagesCategory' => $categoryCurrent->slug,
                                        'COPA03ContentPagesSubCategoryT' => $subcategoryTopic->slug,
                                        'COPA03ContentPagesSubCategoryV' => $subcategoryCurrentV->slug ?? '',
                                    ]) }} #topicos" class="link-full">
                                    </a>
                                    @if ($subcategoryTopic)
                                        <img src="{{ asset('storage/' . $subcategoryTopic->path_image_icon) }}"
                                            alt="Ícone do tópico {{ $subcategoryTopic->title }}" loading="lazy"
            ...
            @if ($subcategoryVideos->count())
                <section class="copa03__videos"  id="videos">
                    ...
                    <nav class="copa03__videos__subcategories">
                        <ul class="copa03__videos__subcategories__swiper-wrapper swiper-wrapper">
                            @foreach ($subcategoryVideos as $subcategoryVideo)
                                <li class="copa03__videos__subcategories__item swiper-slide {{-- {{$subcategoryVideo->id == $subcategoryCurrentV->id ? 'active' : ''}} --}}">
                                    <a href="{{ route('copa03.subcategory.page', [
                                        'COPA03ContentPages' => $contentPage->slug,
                                        'COPA03ContentPagesCategory' => $categoryCurrent->slug,
                                        'COPA03ContentPagesSubCategoryT' => $subcategoryCurrentT->slug ?? 'subcategoria',
                                        'COPA03ContentPagesSubCategoryV' => $subcategoryVideo->slug,
                                    ]) }} #videos" class="link-full">
                                    </a>
                                    @if ($subcategoryVideo->path_image_icon)
                                        <img src="{{ asset('storage/' . $subcategoryVideo->path_image_icon) }}"
                                            alt="Ícone da subcategoria {{ $subcategoryVideo->title }}" loading="lazy"
                        ...
                        @foreach ($videos as $video)
                            @if ($video->path_archive || $video->link)
                                <article class="copa03__videos__main__item" data-fancybox
                                    data-src="{{ $video->path_archive ? asset('storage/' . $video->path_archive) : getUri($video->link) }}">
                                    @if ($video->path_image)
                                        <img src="{{ asset('storage/' . $video->path_image) }}"
                                            alt="Thumbnail do {{ $video->title }}" loading="lazy"
