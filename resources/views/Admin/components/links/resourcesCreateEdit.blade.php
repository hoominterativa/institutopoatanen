@push('createEditCss')
    <!-- Plugins css -->
    <link href="{{url(('Admin/assets/libs/switchery.min.css'))}}" rel="stylesheet" type="text/css" />
    <link href="{{url(('Admin/assets/libs/selectize.bootstrap3.css'))}}" rel="stylesheet" type="text/css" />
    <link href="{{url(('Admin/assets/libs/multi-select.css'))}}" rel="stylesheet" type="text/css" />
    <link href="{{url(('Admin/assets/libs/select2.min.css'))}}" rel="stylesheet" type="text/css" />
    <link href="{{url(('Admin/assets/libs/jquery.bootstrap-touchspin.min.css'))}}" rel="stylesheet" type="text/css" />
    <link href="{{url(('Admin/assets/libs/spectrum.min.css'))}}" rel="stylesheet" type="text/css" />
    <link href="{{url(('Admin/assets/libs/flatpickr.min.css'))}}" rel="stylesheet" type="text/css" />
    <link href="{{url(('Admin/assets/libs/bootstrap-clockpicker.min.css'))}}" rel="stylesheet" type="text/css" />
    <link href="{{url(('Admin/assets/libs/bootstrap-datepicker.min.css'))}}" rel="stylesheet" type="text/css" />
    <link href="{{url(('Admin/assets/libs/dropzone.min.css'))}}" rel="stylesheet" type="text/css" />
    <link href="{{url(('Admin/assets/libs/quill.core.css'))}}" rel="stylesheet" type="text/css" />
    <link href="{{url(('Admin/assets/libs/quill.snow.css'))}}" rel="stylesheet" type="text/css" />
    <link href="{{url(('Admin/assets/libs/dropify.min.css'))}}" rel="stylesheet" type="text/css" />
    <link href="{{url(('Admin/assets/libs/bootstrap-table.min.css'))}}" rel="stylesheet" type="text/css" />
    <link href="{{url(('Admin/assets/libs/sweetalert2.min.css'))}}" rel="stylesheet" type="text/css" />
    <link href="{{url(('Admin/assets/libs/cropper.min.css'))}}" rel="stylesheet" type="text/css" />
    <link href="{{url(('Admin/assets/libs/rcrop.css'))}}" rel="stylesheet" type="text/css" />
@endpush

@push('createEditJs')
    <!-- Plugin js-->
    <script src="{{url(('Admin/assets/libs/parsley.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/i18n/pt-br.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/selectize.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/switchery.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/jquery.multi-select.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/select2.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/jquery.mockjax.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/jquery.autocomplete.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/jquery.bootstrap-touchspin.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/bootstrap-maxlength.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/flatpickr.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/spectrum.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/bootstrap-clockpicker.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/bootstrap-datepicker.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/bootstrap-datepicker.pt-BR.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/jquery.mask.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/autoNumeric.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/dropzone.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/dropify.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/ckeditor.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/bootstrap-table.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/sweetalert2.all.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/cropper.min.js'))}}"></script>
    <script src="{{url(('Admin/assets/libs/rcrop.js'))}}"></script>

    <!-- Pages init js-->
    <script src="{{url(('Admin/assets/js/pages/form-fileuploads.init.js'))}}"></script>
    <script src="{{url(('Admin/assets/js/pages/add-product.init.js'))}}"></script>
    <script src="{{url(('Admin/assets/js/pages/form-advanced.init.js'))}}"></script>
    <script src="{{url(('Admin/assets/js/pages/form-validation.init.js'))}}"></script>
    <script src="{{url(('Admin/assets/js/pages/form-pickers.init.js'))}}"></script>
    <script src="{{url(('Admin/assets/js/pages/form-masks.init.js'))}}"></script>
    <script src="{{url(('Admin/assets/js/pages/ckeditor.init.js'))}}"></script>
    <script src="{{url(('Admin/assets/js/pages/bootstrap-tables.init.js'))}}"></script>
    <script src="{{url(('Admin/assets/js/pages/form-imagecrop.init.js'))}}"></script>
@endpush

