            ...
            <div class="wrapper-links my-2 border px-2 py-3">
                ...
                <div class="row">
                    ...
                    <div class="col-12 col-sm-8">
                        ...
                        {!! Form::url('link_button', (isset($portfolio) && isset($portfolio->link_button) ? getUri($portfolio->link_button) : null), ['class'=>'form-control','parsley-type'=>'url', 'id' => 'targetUrl']) !!}
                    </div>
        ...
        <div class="card card-body" id="tooltip-container">
            <div class="mb-3">
                <div class="container-image-crop">
                    {!! Form::label('inputImage', 'Imagem Antes', ['class' => 'form-label']) !!}
                    <small class="ms-2">Dimensões proporcionais mínimas
                        {{ $cropSetting->path_image_before->width }}x{{ $cropSetting->path_image_before->height }}px!</small>
                    <label class="area-input-image-crop" for="inputImage">
                        {!! Form::file('path_image_before', [
                            'id' => 'inputImage',
                            'class' => 'inputImage',
                            'data-status' => $cropSetting->path_image_before->activeCrop, // px
                            'data-min-width' => $cropSetting->path_image_before->width, // px
                            'data-min-height' => $cropSetting->path_image_before->height, // px
                            'data-box-height' => '180', // Input height in the form
                            'accept' => '.jpg,.jpeg,.png,.gif,.bmp,.tiff,.webp',
                            'data-default-file' => isset($portfolio)
                                ? ($portfolio->path_image_before != ''
                                    ? url('storage/' . $portfolio->path_image_before)
                                    : '')
                                : '',
                        ]) !!}
                    </label>
            ...
            <div class="mb-3">
                <div class="container-image-crop">
                    {!! Form::label('inputImage', 'Imagem Depois', ['class' => 'form-label']) !!}
                    <small class="ms-2">Dimensões proporcionais mínimas
                        {{ $cropSetting->path_image_after->width }}x{{ $cropSetting->path_image_after->height }}px!</small>
                    <label class="area-input-image-crop" for="inputImage">
                        {!! Form::file('path_image_after', [
                            'id' => 'inputImage',
                            'class' => 'inputImage',
                            'data-status' => $cropSetting->path_image_after->activeCrop, // px
                            'data-min-width' => $cropSetting->path_image_after->width, // px
                            'data-min-height' => $cropSetting->path_image_after->height, // px
                            'data-box-height' => '180', // Input height in the form
                            'accept' => '.jpg,.jpeg,.png,.gif,.bmp,.tiff,.webp',
                            'data-default-file' => isset($portfolio)
                                ? ($portfolio->path_image_after != ''
                                    ? url('storage/' . $portfolio->path_image_after)
                                    : '')
                                : '',
                        ]) !!}
                    </label>
