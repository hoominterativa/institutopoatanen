@if (isset($sectionCategory))
    {!! Form::model($sectionCategory, ['route' => ['admin.serv07.section-category.update', $sectionCategory->id], 'class'=>'parsley-validate', 'files' => true]) !!}
    @method('PUT')
@else
    {!! Form::model(null, ['route' => ['admin.serv07.section-category.store'], 'class'=>'parsley-validate', 'files' => true]) !!}
                ...
                <div class="mb-3">
                    <div class="container-image-crop">
                        ...
                        <small class="ms-2">Dimensões proporcionais mínimas
                            {{ $cropSetting->SectionCategory->path_image->width }}x{{ $cropSetting->SectionCategory->path_image->height }}px!</small>
                        <label class="area-input-image-crop" for="inputImage">
                            {!! Form::file('path_image', [
                                'id' => 'inputImage',
                                'class' => 'inputImage',
                                'data-status' => $cropSetting->SectionCategory->path_image->activeCrop, // px
                                'data-min-width' => $cropSetting->SectionCategory->path_image->width, // px
                                'data-min-height' => $cropSetting->SectionCategory->path_image->height, // px
                                'data-box-height' => '225', // Input height in the form
                                'accept' => '.jpg,.jpeg,.png,.gif,.bmp,.tiff,.webp',
                                'data-default-file' => isset($sectionCategory)
                                    ? ($sectionCategory->path_image != ''
                                        ? url('storage/' . $sectionCategory->path_image)
                                        : '')
                                    : '',
                            ]) !!}
                        </label>
