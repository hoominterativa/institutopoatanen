@if (isset($topicCategory))
    {!! Form::model($topicCategory, ['route' => ['admin.serv07.topic-category.update', $topicCategory->id], 'class'=>'parsley-validate', 'files' => true]) !!}
    @method('PUT')
@else
    {!! Form::model(null, ['route' => ['admin.serv07.topic-category.store'], 'class'=>'parsley-validate', 'files' => true]) !!}
                ...
                <div class="mb-3">
                    <div class="container-image-crop">
                        ...
                        <small class="ms-2">Dimensões proporcionais mínimas
                            {{ $cropSetting->TopicCategory->path_image->width }}x{{ $cropSetting->TopicCategory->path_image->height }}px!</small>
                        <label class="area-input-image-crop" for="inputImage">
                            {!! Form::file('path_image', [
                                'id' => 'inputImage',
                                'class' => 'inputImage',
                                'data-status' => $cropSetting->TopicCategory->path_image->activeCrop, // px
                                'data-min-width' => $cropSetting->TopicCategory->path_image->width, // px
                                'data-min-height' => $cropSetting->TopicCategory->path_image->height, // px
                                'data-box-height' => '225', // Input height in the form
                                'accept' => '.jpg,.jpeg,.png,.gif,.bmp,.tiff,.webp',
                                'data-default-file' => isset($topicCategory)
                                    ? ($topicCategory->path_image != ''
                                        ? url('storage/' . $topicCategory->path_image)
                                        : '')
                                    : '',
                            ]) !!}
                        </label>
                    ...
                    <div class="container-image-crop">
                        ...
                        <label class="area-input-image-crop" for="inputImage">
                            {!! Form::file('path_image_icon', [
                                ...
                                'accept' => '.jpg,.jpeg,.png,.gif,.bmp,.tiff,.webp',
                                'data-default-file' => isset($topicCategory)
                                    ? ($topicCategory->path_image_icon != ''
                                        ? url('storage/' . $topicCategory->path_image_icon)
                                        : '')
                                    : '',
                            ]) !!}
                        </label>
