@if (isset($content))
    {!! Form::model($content, ['route' => ['admin.serv05.content.update', $content->id], 'class'=>'parsley-validate', 'files' => true]) !!}
    @method('PUT')
@else
    {!! Form::model(null, ['route' => ['admin.serv05.content.store'], 'class'=>'parsley-validate', 'files' => true]) !!}
            ...
            <div class="card card-body border" id="tooltip-container">
                <div class="mb-3">
                    <div class="container-image-crop">
                        {!! Form::label('inputImage', 'Ícone da seção', ['class' => 'form-label']) !!}
                        <small class="ms-2">Dimensões proporcionais mínimas
                            {{ $cropSetting->Content->path_image_icon->width }}x{{ $cropSetting->Content->path_image_icon->height }}px!</small>
                        <label class="area-input-image-crop" for="inputImage">
                            {!! Form::file('path_image_icon', [
                                'id' => 'inputImage',
                                'class' => 'inputImage',
                                'data-status' => $cropSetting->Content->path_image_icon->activeCrop, // px
                                'data-min-width' => $cropSetting->Content->path_image_icon->width, // px
                                'data-min-height' => $cropSetting->Content->path_image_icon->height, // px
